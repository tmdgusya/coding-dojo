...
fn mission_2_send_message() {
    ...
    println!(
        "Alice Id:{}, Bob ID:{}, Room ID:{}",
        alice_id, bob_id, room_id
    );
...
fn mission_2_room_history() {
    ...
    server
        .send_message(alice_id, room_id, "How are you?")
        .unwrap();
...
fn mission_3_concurrent_user_creation() {
    ...
    for i in 0..10 {
        ...
        let handle = thread::spawn(move || s.create_user(&format!("User{}", i)));
        handles.push(handle);
    }
...
fn mission_3_concurrent_messaging() {
    ...
    for i in 0..5 {
        ...
        let handle = thread::spawn(move || {
            s.send_message(alice_id, room_id, &format!("Message {}", i))
                .unwrap();
        });
        handles.push(handle);
    }
...
fn mission_3_concurrent_read() {
    ...
    for _ in 0..10 {
        let s = server.clone_server();
        let handle = thread::spawn(move || s.get_user(user_id).is_some());
        handles.push(handle);
    }
